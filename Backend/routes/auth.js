...
router.get('/verify', (req, res) => {
    ...
    User.findOneAndUpdate({ verificationToken: token }, { verified: true }, { new: true })
        .then(user => {
            ...
            return res.redirect(`${req.protocol}://${req.headers.host.split(':')[0]}:3000/login`)
        })
        .catch(err => console.log(err));
});
...
router.get('/reset', (req, res) => {
    ...
    User.findOne({ resetPasswordToken: token, resetPasswordExpires: { $gt: Date.now() } })
        .then(user => {
            if (!user) {
            return res.redirect(`${req.protocol}://${req.headers.host.split(':')[0]}:3000/forgot`);
            }
            return res.redirect(`${req.protocol}://${req.headers.host.split(':')[0]}:3000/reset?token=${token}`);
        })
        .catch(err => console.log(err));
});
